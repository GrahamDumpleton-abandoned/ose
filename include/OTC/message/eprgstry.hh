    ...
    //     services in remote process for the corresponding exchange group.
{
  public:
    ...
    OTC_Boolean		containsService(OTC_String const& theServiceAddress)
				{
				  return services_.contains(
				   theServiceAddress);
				}
				// Returns <OTCLIB_TRUE> if service with
    ...
    OTC_Boolean		isServiceGroupMember(
			...
			)
				{
				  return serviceGroups_.contains(
				   theServiceGroup,theServiceAddress);
    				...
    				// Returns <OTCLIB_TRUE> if the service
				// with <theServiceAddress> is in
				// <theServiceGroup>.
