...
class OSE_EXPORT OTC_Exception
    ...
    //
    //     If the environment variable <OTCLIB_EXCEPTIONSTACKTRACE> is
    //     defined when an exception occurs, a stack trace is dumped to the
    //     logger in addition to any location information provided to the
    //     exception class. This only currently works on Solaris and even
    //     then will only give meaningful results for functions which exist
    ...
    // = END<CODE>
{
  public:
    ...
    static void		dumpStackTrace(ostream& theStream);
				// Outputs a stack trace to <theStream>.
				// Only currently works on Solaris and
				// then only producing meaningful results
				// for functions which exist in a shared
				// library.
    ...
    static OTC_Boolean	loggingEnabled();
				// Returns <OTCLIB_TRUE> if output of the
				// details of an exception when raised are
				// displayed using the logger.
  ...
  protected:
    ...
    virtual void	throwIt() const;
				...
				// static member function if exceptions are
				// supported and use of them is enabled.
				// A derived class implementation can check
				// for preprocessor symbol <HAVE_EXCEPTIONS>
				// to determine if exceptions are available.
  ...
  private:
    ...
    static OTC_Boolean	globLogException_;
				// Flag indicating whether details of the
				// exception should be logged even when
				// real exceptions are available.

    static OTC_Boolean	globStackTrace_;
				// Flag indicating if stack trace should
				// be dumped out with file location of
				// exception.
