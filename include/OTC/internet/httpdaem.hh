    ...
    //     <OTC_HttpSession>, <OTC_HttpServlet>
{
  ...
  protected:
    ...
    virtual OTC_Boolean	authorise(OTC_String const& theClientHost);
				// By default returns <OTCLIB_TRUE>. Can
				// be overridden in a derived class to
				// return <OTCLIB_FALSE>, if <theClientHost>
				// should be refused a connection.
				// <theClientHost> will be an IP address.
				// Note that in using this method
				// there is no chance of sending back a
				// response indicating why a client may
				// be refused, the connection will simply
				// be dropped.
