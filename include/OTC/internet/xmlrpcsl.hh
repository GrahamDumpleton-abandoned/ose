...
class OSE_EXPORT OTC_XmlRpcServlet : public OTC_HttpServlet, public OTC_EVAgent
    // = TITLE
    ...
    //     possible size of content in any one XML element.
{
  ...
  private:
    ...
    static OTC_Boolean	decodeRequest_(
			 istream& theStream,
    ...
    static OTC_Boolean	encodeRequest_(
			 ostream& theStream,
    ...
    static OTC_Boolean	encodeResponse_(
			 ostream& theStream,
			 OTC_ROPayload const& theObject
			);
    				// Encodes an XML-RPC response.

    static OTC_Boolean	encodeObject_(
			 ostream& theStream,
			 OTC_ROPayload const& theObject
			);
  ...
  private:
    ...
    static OTC_Boolean	skipProlog_(istream& theStream);
    				// Skips the XML document prolog.

    static OTC_Boolean	skipProperty_(istream& theStream);
    				// Skips an XML element attribute.

    static OTC_Boolean	decodeValue_(
			 istream& theStream,
			 OTC_RWPayload theObject,
			 OTC_String& theWorkarea
			);
    ...
    // <theResult>. The function returns <OTCLIB_TRUE> if successful and
    // <OTCLIB_FALSE> otherwise. A value of <OTCLIB_FALSE> will only be
    // returned when an eof condition is encountered if the function was
    // expecting to read up until some delimiter. The stream is not
    // nessarily checked for failure conditions in all circumstances.
    // If expecting to read up to a specified delimiter, a failure of
    // the stream will be noticed and <OTCLIB_FALSE> returned. In the
    // case where a delimiter is not specified, a failure of the stream
    // will simply stop the reading but not result in <OTCLIB_FALSE>
    // being returned.

    static OTC_Boolean	readName_(
			 istream& theStream,
			 OTC_String& theResult
			);
    				// Reads in a name.

    static OTC_Boolean	readDigits_(
			 istream& theStream,
			 OTC_String& theResult
			);
    				// Reads in string of decimal digits.

    static OTC_Boolean	readHexDigits_(
			 istream& theStream,
			 OTC_String& theResult
			);
    ...
    static OTC_Boolean	readToChar_(
			 istream& theStream,
			 char theChar,
			 OTC_String& theResult
			);
    ...
    static OTC_Boolean	readToString_(
			 istream& theStream,
			 ...
			 OTC_String& theResult
				...
				// <OTCLIB_FALSE> if <theString> is <0> or
				// an empty string. It isn't possible for
				// the delimiter string to contain a null
				// character.

    static OTC_Boolean	decodeToChar_(
			 istream& theStream,
			 char theChar,
			 OTC_String& theResult
			);
    ...
    static OTC_Boolean	decodeString_(
			 istream& theStream,
			 int theDelim,
			 OTC_String& theResult
			)
				...
				// function returning <OTCLIB_TRUE> if the
    ...
    static OTC_Boolean	decodeBinary_(
			 istream& theStream,
			 int theDelim,
			 OTC_String& theResult
			)
				...
				// <OTCLIB_TRUE> if the conversion was
				// successful.
