    ...
    //     function <otclib_hash(...)>.
{
  public:
    ...
    static int		hash(char const* theString, u_int theLength);
				// Returns a hash value for <theString>
				// with <theLength>. If <theString> is
				// <0>, the string is taken to be an empty
				// string.

    friend inline int	otclib_hash(char const* theString, u_int theLength)
    ...
    static int		hash(wchar_t const* theString, u_int theLength);
				{
    ...
    friend inline int	otclib_hash(wchar_t const* theString, u_int theLength)
				{ return OTC_Hash::hash(theString,theLength); }
				// Calls <OTC_Hash::hash()>.
