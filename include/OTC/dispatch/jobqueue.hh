    ...
    //     <OTCEV_IOEvent>, <OTC_Dispatcher>
{
  public:
    ...
    OTC_Boolean		isWaiting();
				// Returns <OTCLIB_TRUE> if the dispatcher is
				// currently waiting to be run, otherwise
				// returns <OTCLIB_FALSE>.

    OTC_Boolean		isRunning();
				// Returns <OTCLIB_TRUE> if the dispatcher is
				// currently running, otherwise returns
				// <OTCLIB_FALSE>. A value of <OTCLIB_FALSE>
				// is also returned if the dispatcher has
				// been stopped, but the <run()> routine has
				// not yet returned.

    OTC_Boolean		isStopped();
				// Returns <OTCLIB_TRUE> if the dispatcher
				// had been running, but had been
				// subsequently stopped, otherwise returns
				// <OTCLIB_FALSE>. It is possible that this
				// will return <OTCLIB_TRUE> while <run()> is
				// still executing as <stop()> may have only
				// been called by the current job and thus
				// <run()> hasn't had an opportunity to
				// return.
  ...
  protected:
    ...
    OTC_Boolean		isPriorityJobsEmpty();
				// Returns <OTCLIB_TRUE> if the priority
				// job queue is empty.
    ...
    OTC_Boolean		isStandardJobsEmpty();
				// Returns <OTCLIB_TRUE> if the executable
				// standard job queue is empty.

    OTC_Job*		nextStandardJob();
				// Returns the next standard job or <0>
				// if there aren't any.

    OTC_Boolean		isPendingStandardJobsEmpty();
				// Returns <OTCLIB_TRUE> if the pending
				// standard job queue is empty.
    ...
    OTC_Boolean		isIdleJobsEmpty();
				// Returns <OTCLIB_TRUE> if the executable
				// idle job queue is empty.

    OTC_Job*		nextIdleJob();
				// Returns the next idle job or <0>
				// if there aren't any.

    OTC_Boolean		isPendingIdleJobsEmpty();
				// Returns <OTCLIB_TRUE> if the pending
				// idle job queue is empty.
