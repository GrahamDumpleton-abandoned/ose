    ...
    //     <OTC_RWPayload>, <OTC_SVPayload>
{
  ...
  public:
    ...
    OTC_Boolean		isValid() const
				...
				// Returns the value <OTCLIB_TRUE> if this
				// reference actually refers to an existing
				// node.
    ...
    OTC_PLReader	operator,(OTC_Boolean& theStatus) const
      				{ return OTC_PLReader(*this,theStatus); }

    // = PROPERTIES

    OTC_Boolean		hasProperty(char const* theName) const;
				// Returns <OTCLIB_TRUE> if the node has a
				// property with <theName>. Will return
				// <OTCLIB_FALSE> if the node reference is
				// invalid. Raises an exception if <theName>
				// is <0>.
  ...
  protected:
    ...
    OTC_Boolean		validProperty(char const* theName) const;
    				// Returns <OTCLIB_TRUE> if <theName> is
				// a valid property name according to the
				// XML rules.
