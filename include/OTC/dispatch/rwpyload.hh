    ...
    //     <OTC_ROPayload>, <OTC_SVPayload>
{
  public:
    ...
    void		operator<<=(OTC_Boolean theValue);
				// Turns this node into a value and sets it
				// to "false" if <theValue> is <OTCLIB_FALSE>
				// and to "true", otherwise. The "type"
				// property will be set to "xsd:boolean" for
				// that value.
    ...
    void		operator<<=(long theValue);
				...
				// "xsd:long" if <theValue> is a 8 byte
    ...
    void		operator<<=(u_long theValue);
				...
				// the string representation of <theValue>.
				// The "type" property will be set to
				// "xsd:unsignedLong" if <theValue> is a 8
    ...
    OTC_ROPayload	sibling(char const* thePath, u_int theLength) const;
    ...
    OTC_RWPayload	sibling(char const* thePath, u_int theLength);
				// Returns a new reference to the node below
				// this node as described by <thePath> where
				// <thePath> has <theLength>. <thePath> is
    ...
    OTC_Boolean		decode(istream& theStream, u_int theCapacity=0);
				...
				// decoding is successful <OTCLIB_TRUE> will
				// also be returned and <OTCLIB_FALSE>
				// otherwise. To improve decoding
				// performance, <theCapacity> should be a
				// guess at the maximum amount of memory
    ...
    OTC_Boolean		decode(
			 ...
			 u_int theCapacity=0
				...
				// will be deemed as have failed. If
				// the decoding is successful <OTCLIB_TRUE>
				// will also be returned and <OTCLIB_FALSE>
				// otherwise with the node type set to
				// "invalid". To improve decoding performance,
				// <theCapacity> should be a guess at the
				// maximum amount of memory which would need
				// to be used to decode any one element in
				// the XML. By providing this you avoid
				// unecessary memory reallocations and memory
				// copying. If <theCapacity> is <0>, a default
				// value corresponding to the size of
				// <theString> is used. In general with this
				// method there should be no reason to
				// override the value the default for
				// <theCapacity> unless the input is very big
				// but the largest elements are quite small.

  private:

    // All the following return <OTCLIB_TRUE> if successful or <OTCLIB_FALSE>
    // if a problem was encountered.

    OTC_Boolean		decode_(istream& theStream, OTC_String& theWorkarea);
				// Decodes the data model from its serialised
				// XML form as present on <theStream>. If the
				// decoding fails, <theStream> will be left
				// with the fail bit set and the node will be
				...
				// decoding is successful <OTCLIB_TRUE> will
				// also be returned and <OTCLIB_FALSE>
				// otherwise. <theWorkarea> should be a
  ...
  private:
    ...
    static OTC_Boolean	initialised_;
				// Flag to indicate input buffer size
				// has been initialised.

    static u_int	capacity_;
