    ...
    //     assignment.
{
  public:
    ...
    // = INITIALISATION
			...
			OTC_RString(char const* theString, u_int theLength);
				...
				// <OTCLIB_TRUE>. You should not modify
				// the actual string contents for the type
				// of object created by this class. If you
				// do you will corrupt the symbol table
				// indexing resulting in bogus entries in
				// the symbol table. This constructor is
				// primarily intended only to be used by
				// the <OTC_Symbol> class. The memory
				// allocated as a result of this constructor
				// is always in transient memory. That is
				// you cannot use this constructor to store
				// data in an ObjectStore database. Only
				// enough memory to hold the string is
				// allocated.
    ...
    OTC_Boolean		isSymbol() const
				...
				// Returns <OTCLIB_TRUE> if the string
				// is indexed by the symbol table.
    ...
    static OTC_Boolean	symbolExists(char const* theString, u_int theLength);
				// Returns <OTCLIB_TRUE> if the string
				// described by <theString> and <theLength>
				// is currently listed in the symbol
				// database.
  ...
  private:
    ...
    char*		allocate_(u_int theSize) const;
    				// Allocates a piece of memory of <theSize>.

    void		release_(char* theMemory, u_int theSize) const;
