    ...
    //     <OTC_TString>
{
  public:
    ...
    // and thus are subject to change. They are only here for backward
    // compatability.
    ...
    static int		rank(
			 ...
			 OTC_SCmpType theType=OTCLIB_EXACTMATCH
				...
				// <OTCLIB_IGNORECASE>. Ordering is
				// based on the ASCII values in the string
				// and not on any locale based ordering
				// convention.

    static int		hash(char const* theString, u_int theNum)
				{ return OTC_Hash::hash(theString,theNum); }
    ...
    OTC_String		besides(u_int theStart, u_int theLength) const
  ...
  public:
    ...
    friend OSE_EXPORT OTC_Boolean	operator==(
				 char c1,
				 OTC_String const& s2
				)
				{
  ...
  private:
    ...
    OTC_String		section_(int theStart, int theLength) const;

    OTC_String		except_(int theStart, u_int theLength) const;

    void		upper_(int theStart, u_int theLength);

    void		lower_(int theStart, u_int theLength);
