...
class OSE_EXPORT OTC_Regex : public OTC_Pattern
    // = TITLE
    ...
    //	   <OTC_Pattern>
{
  ...
  public:
    ...
    OTC_Boolean		match(char const* theString);
				...
				// matches the most recently compiled
				// pattern. If no pattern had been compiled
				// then <OTCLIB_FALSE> is returned. If
				// <theString> is <0> then <OTCLIB_FALSE>
				// is always returned.
    ...
    u_int		length(u_int theIndex=0) const;
  ...
  private:
    ...
    OTC_Boolean		sta;
				// <OTCLIB_TRUE> if there is a pattern
				// currently compiled.
    ...
    OTC_Boolean		valid_;
				// Is <OTCLIB_TRUE> if pattern compiled
				// correctly.
