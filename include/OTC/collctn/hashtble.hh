    ...
    //     <OTC_HashEntry>
{
  public:
    ...
    OTC_Boolean		isEmpty() const
				...
				// Returns <OTCLIB_TRUE> if the hash table
				// contains no entries which are occupied.
  ...
  private:
    ...
    u_int		start_(int theHashValue) const
				{ return mask_(3 * theHashValue); }
				// Special version of <mask_()> for finding
				// starting position in hash vector to look
				// for item. This is needed so we can adjust
				// hash value by multiplying it by three to
				// avoid clumping when sequential integers
				// are used as keys.
