...
/* ------------------------------------------------------------------------- */
template<class T>
os_typespec* OTC_IdxVecCursor<T>::typespec()
{
  static os_typespec ts("OTC_IdxVecCursor<int>");
  return &ts;
}
#endif
    ...
    protection_(theProtection)
{
  OTCLIB_ENSURE_FN((theVector != 0 || theSize == 0),
   ...
   "invalid vector");
  OTCLIB_ENSURE_FN((theCount != 0 || theProtection == OTCLIB_FALSE),
   "OTC_IdxVecCursor<T>::OTC_IdxVecCursor(T*, u_int, OTC_Count, OTC_Direction)",
...
{
  OTCLIB_ENSURE_FN((isValid() != OTCLIB_FALSE),
...
/* ------------------------------------------------------------------------- */
template<class T>
OTC_Boolean OTC_IdxVecCursor<T>::isValid() const
{
