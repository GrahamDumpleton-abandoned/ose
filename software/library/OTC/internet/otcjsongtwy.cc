...
#pragma implementation "OTC/internet/jsongtwy.hh"
#endif
#endif
...
static OTC_TraceTag& MODULE_TRACETAG()
{
  ...
  if (TAG == 0)
    TAG = new OTC_TraceTag("OTC_JsonRpcGateway");
...
/* ------------------------------------------------------------------------- */
OTC_JsonRpcGateway::~OTC_JsonRpcGateway()
{
  OTCLIB_MARKBLOCK(MODULE,"OTC_JsonRpcGateway::~OTC_JsonRpcGateway()");
...
/* ------------------------------------------------------------------------- */
OTC_HttpServlet* OTC_JsonRpcGateway::servlet(OTC_HttpSession* theSession)
{
  OTCLIB_MARKBLOCK(MODULE,"OTC_JsonRpcGateway::servlet()");
  ...
  if (type_ == OTCLIB_SERVICE_NAME || type_ == OTCLIB_SERVICE_ADDRESS)
  {
    ...
    {
      theServlet = new OTC_JsonRpcServlet(theSession,0);
      OTCLIB_ASSERT_M(theServlet != 0);
    }
  }
  else
...
/* ------------------------------------------------------------------------- */
void OTC_JsonRpcGateway::handle(OTC_Event* theEvent)
{
  OTCLIB_MARKBLOCK(MODULE,"OTC_JsonRpcGateway::servlet()");
