...
void test1()
{
  ...
  OTC_JsonRpcGateway theGateway1("public",OTCLIB_SERVICE_GROUP);
  theDaemon.attach("/group",&theGateway1);

  OTC_JsonRpcGateway theGateway2("",OTCLIB_SERVICE_GROUP);
  theDaemon.attach("/registry",&theGateway2);

  OTC_JsonRpcGateway theGateway3("time",OTCLIB_SERVICE_NAME);
  theDaemon.attach("/name",&theGateway3);

  OTC_JsonRpcGateway theGateway4(theService2.serviceAddress(),
   OTCLIB_SERVICE_ADDRESS);
  theDaemon.attach("/address",&theGateway4);

  OTC_JsonRpcGateway theGateway5("*",OTCLIB_SERVICE_REGISTRY);
  theDaemon.attach("/wildcard",&theGateway5);

  OTC_JsonRpcGateway theGateway6("",OTCLIB_SERVICE_REGISTRY);
  theDaemon.attach("/local",&theGateway6);

  OTC_JsonRpcGateway theGateway7("network",OTCLIB_SERVICE_REGISTRY);
  theDaemon.attach("/network",&theGateway7);

  OTC_JsonRpcGateway theGateway8("unknown",OTCLIB_SERVICE_REGISTRY);
