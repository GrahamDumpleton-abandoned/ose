...
)
{
  ...
  atexit(otclib_winsock_cleanup);
    ...
    if (!gethostbyname_r(hostname(),&hostEntBuf,&hostEntData))
      h = &hostEntBuf;
#else
    ...
    h = gethostbyname_r(hostname(),&hostEntBuf,stringBuf,
  ...
  if (!gethostbyname_r(hostname(),&hostEntBuf,stringBuf,
