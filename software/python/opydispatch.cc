...
/* ------------------------------------------------------------------------- */
bool OPY_Dispatcher::gInitialised_ = false;
OTC_HMap<PyInterpreterState*,OPY_Dispatcher*>* OPY_Dispatcher::gInstances_ = 0;
OTC_HMap<PyInterpreterState*,PyThreadState*>* OPY_Dispatcher::gStates_ = 0;
...
PyObject* OPY_Dispatcher::instance(PyObject* theModule, PyObject* theArgs)
{
  ...
  if (!gInitialised_)
  {
    ...
    gStates_ = new OTC_HMap<PyInterpreterState*,PyThreadState*>();
    gInstances_ = new OTC_HMap<PyInterpreterState*,OPY_Dispatcher*>();
  }
  ...
  if (!gStates_->contains(theInterpreterState))
  {
    PyThreadState* tmpThreadState = 0;
    tmpThreadState = PyThreadState_New(theInterpreterState);
    gStates_->add(theInterpreterState,tmpThreadState);
  }
#endif

  if (gInstances_->contains(theInterpreterState))
    Py_XDECREF(gInstances_->item(theInterpreterState));

  if (!gInstances_->contains(theInterpreterState))
  {
    tmpInstance = new OPY_Dispatcher();
    gInstances_->add(theInterpreterState,tmpInstance,OTCLIB_REPLACE);
    Py_XINCREF(tmpInstance);
  }
  else
    tmpInstance = gInstances_->item(theInterpreterState);

  return (PyObject*)tmpInstance;
}
...
/* ------------------------------------------------------------------------- */
void OPY_Dispatcher::enterPython(PyInterpreterState* theInterpreterState)
{
#ifdef WITH_THREAD
  PyThreadState* tmpThreadState = 0;

  if (gStates_->contains(theInterpreterState))
    tmpThreadState = gStates_->item(theInterpreterState);
  else
    Py_FatalError("OPY_Dispatcher::enterPython - tstate unavailable");

  PyEval_AcquireThread(tmpThreadState);
#endif

  // XXX Should I really clear this.

  PyErr_Clear();
}

/* ------------------------------------------------------------------------- */
void OPY_Dispatcher::leavePython(PyInterpreterState* theInterpreterState)
{
#ifdef WITH_THREAD
  PyThreadState* tmpThreadState = 0;
  ...
  if (gStates_->contains(theInterpreterState))
    ...
    Py_FatalError("OPY_Dispatcher::leavePython - tstate unavailable");

  PyEval_ReleaseThread(tmpThreadState);
#endif
}
