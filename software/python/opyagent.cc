...
)
{
  ...
  {
    // At most there can only be one thread. If there is
    // ever going to be more than one then there need to
    // be an equal number of thread states as there are
    // threads.
...
void OPY_Agent::execute(OTC_Event* theEvent)
{
  ...
  // Use interpreter global thread state in this method
  // so that when reference count decrement occurs, if
  // it is the last reference we aren't deleting our own
  // thread state in the destructor.
