...
if os.environ.has_key("OSE_ROOT"):
  OSE_ROOT = os.environ["OSE_ROOT"]
else:
  OSE_ROOT = "@OSE_ROOT@"

OSE_RELEASE_DIRECTORY = "@OSE_RELEASE_DIRECTORY@"

if os.environ.has_key("OSE_HOST"):
  OSE_HOST = os.environ["OSE_HOST"]
else:
  OSE_HOST = "UNKNOWN"
...
if OSE_HOST_HOME[0] != "@":
  sys.path.insert(0,OSE_HOST_HOME+"/lib/python")

# Import "socket" so that WinSock
# gets initialised on Win32 platform.
...
# Thread synchronisation wrappers.
...
class Agent:
  ...
  def synchronisedObject(self):
    return _SynchronisedObject(self)

  def acquireObject(self):
  ...
  def scheduleAction(self,callback,mode=STANDARD_JOB,name=None):
    ...
    if type(mode) is types.IntType:
      self._actionCallbacks[name] = callback
  ...
  def cancelAction(self,name):
    ...
    elif self._actionCallbacks.has_key(name):
  ...
  def _handleAction(self,name):
	...
	callback = self._actionCallbacks[name]
	del self._actionCallbacks[name]
	nargs = callback.im_func.func_code.co_argcount-1
	if nargs == 0:
	  callback()
  ...
  def _handleShutdown(self,category):
    try:
      if self._shutdownCallback != None:
...
class Service(Monitor):
  ...
  def __init__(self,name="",audience="*"):
    inner = _ose.new_Service(self,name,audience)
  ...
  def destroyReferences(self):
    ...
    self._currentFailure = None
    self._currentReport = None
    Monitor.destroyReferences(self)
  ...
  def suspendResponse(self,callback):
...
class RpcGateway(HttpServer):
  ...
  def __init__(self,group):
    inner = _ose.new_RpcGateway(self,group)
