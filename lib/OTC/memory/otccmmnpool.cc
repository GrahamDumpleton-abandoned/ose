...
OTC_Boolean OTC_CommonPool::init_ = OTCLIB_FALSE;
OTC_Boolean OTC_CommonPool::disable_ = OTCLIB_FALSE;
OTC_Pool** OTC_CommonPool::pools_ = 0;
u_int OTC_CommonPool::numPools_ = 0;
size_t OTC_CommonPool::increment_ = 0;
size_t OTC_CommonPool::largest_ = 0;
u_int* OTC_CommonPool::index_ = 0;
...
void OTC_CommonPool::initialise()
{
  ...
  if (getenv("OTCLIB_NOCOMMONPOOL") != 0)
    disable_ = OTCLIB_TRUE;

  if (disable_ == OTCLIB_FALSE && pools_ == 0)
  {
    ...
    index_ = new u_int[largest_+1];
    OTCLIB_ASSERT_M(index_ != 0);

    index_[0] = 0;
    for (u_int j=1; j<=largest_; j++)
