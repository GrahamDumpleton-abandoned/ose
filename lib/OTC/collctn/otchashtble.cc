...
/* ------------------------------------------------------------------------- */
os_typespec* OTC_HashTable::typespec()
{
  static os_typespec ts("OTC_HashTable");
  return &ts;
}
#endif
...
#if defined(ENV_OSTORE)
  hashTable_ = new (theLocality,OTC_HashEntry::get_os_typespec(),tableSize_)
   OTC_HashEntry[tableSize_];
#else
...
#if defined(ENV_OSTORE)
  hashTable_ = new (theLocality,OTC_HashEntry::get_os_typespec(),
   tableSize_) OTC_HashEntry[tableSize_];
#else
  hashTable_ = new OTC_HashEntry[tableSize_];
#endif
  OTCLIB_ASSERT_M(hashTable_ != 0);
  ...
  for (u_int j=0; j<theTableSize; j++)
  {
    ...
    if (oldEntry->isOccupied() != OTCLIB_FALSE)
    {
      ...
      for (u_int i=0; i<=tableSize_; i++)
      {
        ...
        if (tmpEntry->isEmpty() != OTCLIB_FALSE)
        {
