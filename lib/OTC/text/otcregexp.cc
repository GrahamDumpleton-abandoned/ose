...
/* ------------------------------------------------------------------------- */
os_typespec* OTC_RegexpStruct::typespec()
{
  static os_typespec ts("OTC_RegexpStruct");
  return &ts;
}

/* ------------------------------------------------------------------------- */
os_typespec* OTC_Regexp::typespec()
{
  static os_typespec ts("OTC_Regexp");
  return &ts;
}
#endif
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_Regexp::match(char const* theString)
{
  string_ = theString;
  OTC_Boolean matched = regexec(regexp_,theString);
  return matched;
}
...
/* ------------------------------------------------------------------------- */
u_int OTC_Regexp::length(u_int theIndex) const
{
  if (regexp_ && theIndex < maxSubPatterns_
   && regexp_->startp[theIndex] != 0)
  {
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_Regexp::isValid() const
{
  // return regexp_ != 0 ? OTCLIB_TRUE : OTCLIB_FALSE;
  return error_ == 0 ? OTCLIB_TRUE : OTCLIB_FALSE;
}
...
#if defined(ENV_OSTORE)
	r = new (theLocality,OTC_RegexpStruct::get_os_typespec())
	 OTC_RegexpStruct;
	OTCLIB_ASSERT_M(r != 0);
	memset((char*)r,0,sizeof(OTC_RegexpStruct));
	r->program = new (theLocality,os_typespec::get_char(),regsize+1)
	 char[regsize+1];
	OTCLIB_ASSERT_M(r->program != 0);
	memset(r->program,0,regsize);
	r->startp = new (theLocality,OTC_TypeSpec<char*>::typespec(),
	 maxSubPatterns_) char const*[maxSubPatterns_];
	OTCLIB_ASSERT_M(r->startp != 0);
	memset((char*)r->startp,0,sizeof(r->startp[0])*maxSubPatterns_);
	r->endp = new (theLocality,OTC_TypeSpec<char*>::typespec(),
	 maxSubPatterns_) char const*[maxSubPatterns_];
