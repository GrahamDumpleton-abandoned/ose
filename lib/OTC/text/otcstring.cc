...
OTC_Boolean OTC_String::globInitialised_ = OTCLIB_FALSE;
  ...
  : data_(theNum)
{
  OTCLIB_ENSURE_FN((theNum <= theString.length()),
   "OTC_String(OTC_String const&, u_int)",
   "invalid character count");
...
)
{
  ...
  u_int theLength = theString.data_.length();
  u_int theCapacity = theString.data_.capacity();
  ...
  while (theEnd == OTCLIB_FALSE)
  {
    ...
    if ((int)ins.gcount() != (int)theSpace)
    {
      theEnd = OTCLIB_TRUE;
    }
    else
...
OTC_String& OTC_String::readFile(istream& theStream)
{
  ...
  u_int theLength = data_.length();
  u_int theCapacity = data_.capacity();
  u_int theSpace = theCapacity - theLength;
  ...
  while (theEnd == OTCLIB_FALSE)
  {
    ...
    if ((int)theStream.gcount() != (int)theSpace)
    {
...
OTC_String& OTC_String::readToken(istream& theStream)
{
  ...
  u_int theCapacity = theData.capacity();
...
OTC_String& OTC_String::assign(OTC_String const& theString, u_int theNum)
{
  OTCLIB_ENSURE_FN((theNum <= theString.data_.length()),
   "OTC_String::assign(OTC_String const&, u_int)",
   "invalid length");
...
OTC_String& OTC_String::truncate(u_int theIndex)
{
  ...
  int theNum = data_.length()-theIndex;

  if (theNum == 0)
    return *this;

  OTCLIB_ENSURE_FN((theNum > 0),
   "OTC_String::truncate(u_int)",
   "index out of range");
...
/* ------------------------------------------------------------------------- */
int OTC_String::index_(u_int theIndex, char theChar, u_int theNum) const
{
  OTCLIB_ENSURE_FN((theNum > 0),
   "OTC_String::index_(u_int, char, u_int)",
  ...
  if (tmpString != 0)
  {
    int anIndex = tmpString - data_.string();
    if (theNum == 1)
    {
      return anIndex;
    }
...
/* ------------------------------------------------------------------------- */
int OTC_String::rindex_(char theChar, u_int theNum) const
{
  OTCLIB_ENSURE_FN((theNum > 0),
   "OTC_String::rindex_(char, u_int)",
   "bad occurrence number");

  if (data_.length() == 0)
    return -1;

  int theIndex = data_.length() - 1;
  while (theIndex >= 0)
  {
...
{
  OTCLIB_ENSURE_FN((theNum > 0),
   "OTC_String::index_(u_int, OTC_CBitSet const&, u_int)",
   "bad occurrence number");
  ...
  char const* tmpString = data_.string() + theIndex;
  u_int theLength = data_.length() - theIndex;
...
/* ------------------------------------------------------------------------- */
int OTC_String::rindex_(OTC_CBitSet const& theCharSet, u_int theNum) const
{
  OTCLIB_ENSURE_FN((theNum > 0),
   "OTC_String::rindex_(OTC_CBitSet const&, u_int)",
   "bad occurrence number");
...
) const
{
  OTCLIB_ENSURE_FN((theLength <= theString.data_.length()),
   "OTC_String::rank(OTC_String const&, u_int, OTC_SRankType)",
   "invalid length");
...
{
  OTCLIB_ENSURE_FN((theIndex <= data_.length()),
   "OTC_String::compare(u_int, char const*, OTC_SCmpType)",
   "index out of range");
...
{
  OTCLIB_ENSURE_FN((theIndex <= data_.length()),
   "OTC_String::compare(u_int, char const*, u_int, OTC_SCmpType)",
   "index out of range");
...
{
  OTCLIB_ENSURE_FN((theIndex <= data_.length()),
   "OTC_String::compare(u_int, OTC_String const&, OTC_SCmpType)",
   "index out of range");
...
{
  OTCLIB_ENSURE_FN((theIndex <= data_.length()),
   "OTC_String::compare(u_int, OTC_String const&, u_int, OTC_SCmpType)",
   "index out of range");
...
#if defined(OTCLIB_50000_COMPATABILITY)
/* ------------------------------------------------------------------------- */
int OTC_String::rank(
...
)
{
  ...
  return OTC_Rank::rank(theString1,theLength1,
   theString2,theLength2,theRankType);
}
#endif
...
)
{
  OTCLIB_ENSURE_FN((theStart >= 0 && theStart+theLength <= data_.length()),
   "OTC_String::replace_(int, u_int, char, u_int)",
   "invalid range");

  u_int oldLength = data_.length();
  ...
  if (data_.isShared())
  {
    ...
    {
      OTC_RString tmpData(data_.length()+theNum-theLength,this);
...
{
  OTCLIB_ENSURE_FN((theStart >= 0 && theStart+theLength <= data_.length()),
   "OTC_String::replace_(int, u_int, char const*, u_int)",
   "invalid range");
...
{
  OTCLIB_ENSURE_FN((theNum <= theString.data_.length()),
   "OTC_String::replace_(int, u_int, OTC_String const&, u_int)",
   "invalid range");
...
/* ------------------------------------------------------------------------- */
OTC_String OTC_String::section_(int theStart, int theLength) const
{
  OTCLIB_ENSURE_FN((theLength >= 0),
   "OTC_String::section_(int, int)",
   "invalid range");
...
/* ------------------------------------------------------------------------- */
void OTC_String::upper_(int theStart, u_int theLength)
{
  OTCLIB_ENSURE_FN((theStart >= 0 && theStart+theLength <= data_.length()),
   "OTC_String::upper_(int, u_int)",
   "invalid range");
...
/* ------------------------------------------------------------------------- */
void OTC_String::lower_(int theStart, u_int theLength)
{
  OTCLIB_ENSURE_FN((theStart >= 0 && theStart+theLength <= data_.length()),
   "OTC_String::lower_(int, u_int)",
   "invalid range");
...
) const
{
  ...
  OTCLIB_ENSURE_FN((theIndex < data_.length()),
   "OTC_String::index_(u_int, char const*, u_int)",
   "index out of range");

  u_int theLength1 = data_.length() - theIndex;
  ...
  if (theStart != 0)
  {
    u_int anIndex = theStart - data_.string();
    return anIndex;
  }
