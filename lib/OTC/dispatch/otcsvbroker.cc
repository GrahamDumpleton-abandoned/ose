...
OTC_SVBroker::OTC_SVBroker(
 ...
 char const* theServiceAudience
  ...
  : agent_(theAgent), serviceBinding_(0)
{
  OTCLIB_MARKBLOCK(MODULE,"OTC_SVBroker::OTC_SVBroker()");
  ...
  serviceBinding_ = registry()->addServiceBinding(theServiceName,
...
void OTC_SVBroker::handle(OTC_Event* theEvent)
{
  ...
  else if (theEvent->type() == OTCEV_Envelope::typeId())
  {
     ...
     theEnvelope->subject() == "$S::SUBSCRIPTION-WITHDRAWN")
    {
      ...
      {
	OTC_String theHeader;
	theHeader = theEnvelope->header();

	istrstream ins1((char*)theHeader.string(),theHeader.length());

	OTC_String tmpLine;
	OTC_String theSubjectPattern;
	  ...
	  if (theEnvelope->subject() == "$S::SUBSCRIPTION-REQUESTED")
	  {
		...
		if (matchSubject(theSubjectPattern,theEntry->key()))
		{
		  ...
		  else
		  {
		    ...
		    theHeader += theEntry->key();
		    theHeader += EOL;
		    theHeader += "SubjectPattern: ";
		    theHeader += theSubjectPattern;
		    theHeader += EOL;
...
)
{
  ...
  OTC_String tmpContent;
  char const* tmpContentType;

  tmpContent = theContent.encode();
  tmpContentType = "text/xml";
  ...
  theConversationId = theEnvelope->conversationId();
...
)
{
  ...
  OTC_String tmpSubjectPattern;

  tmpServiceName = theServiceName;
  tmpSubjectPattern = theSubjectPattern;
...
)
{
  ...
  tmpServiceAddress = theServiceAgent->serviceAddress();
  tmpSubjectPattern = theSubjectPattern;
...
)
{
  ...
  if (agentSubscriptions_.contains(tmpServiceAddress,tmpSubjectPattern))
  {
    ...
    if (!agentSubscriptions_.containsKey(tmpServiceAddress))
    {
      ...
      // subscribed to use, we can unsubscribe
      // to its existance.
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_SVBroker::matchSubject(
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_SVBroker::builtinRequest_(OTCEV_Request* theRequest)
{
  return OTCLIB_FALSE;
