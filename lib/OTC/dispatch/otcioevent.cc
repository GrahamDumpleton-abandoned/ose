...
{
  if (validFd_(theFd) == OTCLIB_FALSE)
    return 0;
...
{
  OTCLIB_ENSURE_FN((validFd_(theFd) != OTCLIB_FALSE),
...
{
  if (theAgentId == 0 || validFd_(theFd) == OTCLIB_FALSE)
    return;
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTCEV_IOEvent::validFd_(int theFd)
{
#if defined(SYS_UNIX)
