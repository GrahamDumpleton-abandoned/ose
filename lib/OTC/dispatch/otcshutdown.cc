...
OTC_Boolean OTCEV_Shutdown::gShutdownArrived_ = OTCLIB_FALSE;
...
void OTCEV_Shutdown::resume()
{
  ...
  if (gDelayCount_ != 0)
  {
    ...
    if (gDelayCount_ == 0)
    {
      if (gShutdownArrived_ != OTCLIB_FALSE)
      {
	OTC_LogStream theLogger;
	theLogger.setLevel(OTCLIB_LOG_INFO);
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTCEV_Shutdown::isScheduled()
{
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTCEV_Shutdown::isSuspended()
{
  OTC_Boolean theResult;
  theResult = OTCLIB_FALSE;
...
void OTCEV_Shutdown::signalCallback_(OTC_Event* theEvent, int theAgentId)
{
  ...
  {
    OTC_Boolean inProgress = OTCLIB_FALSE;
    ...
    if (gShutdownAgent_ != 0)
      inProgress = OTCLIB_TRUE;

    xxxMutex.release();

    mutex_.unlock();

    if (inProgress != OTCLIB_FALSE)
    {
      OTC_LogStream theLogger;
      theLogger.setLevel(OTCLIB_LOG_INFO);
...
void OTCEV_Shutdown::shutdownCallback_(OTC_Event* theEvent, int theAgentId)
{
  ...
  else if (theEvent->type() == OTCEV_Action::typeId())
  {
    ...
    gShutdownArrived_ = OTCLIB_FALSE;
