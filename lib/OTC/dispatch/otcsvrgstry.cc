	 ...
	 theBinding->serviceLocation() == OTCLIB_SERVICE_REMOTE))
	{
	  ...
	  while (theGroups.isValid())
	  {
	    theEvent = new OTCEV_GroupAnnouncement(theBinding,
	     theGroups.item(),OTCLIB_SERVICE_AVAILABLE,OTCLIB_SERVICE_REGISTRY);
	    OTCLIB_ASSERT_M(theEvent != 0);
...
{
  OTCLIB_MARKBLOCK(MODULE,
   "OTC_SVRegistry::subscribeServiceName(int,char const*)");
  ...
  OTC_String tmpServiceName;
  tmpServiceName = theServiceName;
  ...
  if (!subServiceName_.contains(tmpServiceName,theAgentId))
    subServiceName_.add(tmpServiceName,theAgentId);

  OTC_Iterator<OTC_ServiceBinding*> theServices;
  theServices = mapServiceName_.items(tmpServiceName);
...
{
  OTCLIB_MARKBLOCK(MODULE,
   "OTC_SVRegistry::unsubscribeServiceName(int,char const*)");
...
{
  OTCLIB_MARKBLOCK(MODULE,
   "OTC_SVRegistry::subscribeServiceAddress(int,char const*,char const*)");
  ...
  tmpServiceAddress = theServiceAddress;

  if (!mapServiceAddress_.contains(tmpServiceAddress,theAgentId))
    mapServiceAddress_.add(tmpServiceAddress,theAgentId);
  ...
  if (!subServiceAddress_.contains(tmpServiceAddress))
  {
    ...
    subServiceAddress_.add(tmpServiceAddress,theSubscribers);
  }
  else
    theSubscribers = subServiceAddress_.item(tmpServiceAddress);

  OTC_String tmpSubscription;
  tmpSubscription = theSubscription;

  theSubscribers->addSubscription(theAgentId,tmpSubscription);
  ...
  if (services_.contains(tmpServiceAddress))
  {
    ...
    theEvent = new OTCEV_ServiceAnnouncement(
     ...
     OTCLIB_SERVICE_ADDRESS,tmpSubscription);
    OTCLIB_ASSERT_M(theEvent != 0);
...
{
  OTCLIB_MARKBLOCK(MODULE,
   "OTC_SVRegistry::unsubscribeServiceAddress(int,char const*,char const*)");
  ...
  if (subServiceAddress_.contains(tmpServiceAddress))
  {
    ...
    theSubscribers->removeSubscription(theAgentId,tmpSubscription);

    if (theSubscribers->numSubscriptions(theAgentId) == 0)
      mapServiceAddress_.remove(tmpServiceAddress,theAgentId,OTCLIB_UNSAFE);

    if (theSubscribers->numSubscriptions() == 0)
      subServiceAddress_.remove(tmpServiceAddress);
  }
...
)
{
  ...
  while (theAddresses.isValid())
  {
    ...
    theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
     theServiceGroup,OTCLIB_SERVICE_AVAILABLE,OTCLIB_SERVICE_GROUP);
    OTCLIB_ASSERT_M(theEvent != 0);
...
void OTC_SVRegistry::removeServiceBinding(char const* theServiceAddress)
{
  ...
  if (services_.contains(theServiceAddress))
  {
    ...
    OTC_String tmpServiceAddress;
    tmpServiceAddress = tmpBinding->serviceAddress();
	...
	{
	  theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
	   theGroups.item(),OTCLIB_SERVICE_WITHDRAWN,OTCLIB_SERVICE_GROUP);
	  OTCLIB_ASSERT_M(theEvent != 0);
	...
	{
	  theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
	   theGroups.item(),OTCLIB_SERVICE_WITHDRAWN,OTCLIB_SERVICE_ADDRESS);
	  OTCLIB_ASSERT_M(theEvent != 0);
	...
	theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
	 theGroups.item(),OTCLIB_SERVICE_WITHDRAWN,OTCLIB_SERVICE_REGISTRY);
	OTCLIB_ASSERT_M(theEvent != 0);
...
)
{
  ...
  OTC_String tmpServiceGroup;
  OTC_String tmpServiceAddress;

  tmpServiceGroup = theServiceGroup;
  tmpServiceAddress = theServiceAddress;
  ...
  if (!mapServiceGroup_.contains(tmpServiceGroup,tmpServiceAddress))
  {
    ...
    theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
     tmpServiceGroup,OTCLIB_SERVICE_AVAILABLE,OTCLIB_SERVICE_REGISTRY);
    OTCLIB_ASSERT_M(theEvent != 0);
    ...
    {
      theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
       tmpServiceGroup,OTCLIB_SERVICE_AVAILABLE,OTCLIB_SERVICE_ADDRESS);
      OTCLIB_ASSERT_M(theEvent != 0);
    ...
    {
      theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
       tmpServiceGroup,OTCLIB_SERVICE_AVAILABLE,OTCLIB_SERVICE_GROUP);
      OTCLIB_ASSERT_M(theEvent != 0);
...
)
{
  ...
  if (mapServiceGroup_.contains(tmpServiceGroup,tmpServiceAddress))
  {
    ...
    {
      theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
       tmpServiceGroup,OTCLIB_SERVICE_WITHDRAWN,OTCLIB_SERVICE_GROUP);
      OTCLIB_ASSERT_M(theEvent != 0);
    ...
    {
      theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
       tmpServiceGroup,OTCLIB_SERVICE_WITHDRAWN,OTCLIB_SERVICE_ADDRESS);
      OTCLIB_ASSERT_M(theEvent != 0);
    ...
    theEvent = new OTCEV_GroupAnnouncement(tmpBinding,
     tmpServiceGroup,OTCLIB_SERVICE_WITHDRAWN,OTCLIB_SERVICE_REGISTRY);
    OTCLIB_ASSERT_M(theEvent != 0);
