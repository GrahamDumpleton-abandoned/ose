...
OTC_EVThreadPool::~OTC_EVThreadPool()
{
  ...
  if (threads_ != 0)
  {
    ...
    for (i=0; i<numThreads_; i++)
      thr_join(threads_[i],0,&theResult);
#endif
#if defined(HAVE_POSIX_THREADS)
    u_int i;
    void* theResult = 0;
    for (i=0; i<numThreads_; i++)
      pthread_join(threads_[i],&theResult);
#endif
...
void OTC_EVThreadPool::startup()
{
  ...
  if (numThreads_ != 0 && threads_ == 0)
  {
...
#if defined(HAVE_SOLARIS_THREADS)
    threads_ = new thread_t[numThreads_];
    ...
    for (u_int i=0; i<numThreads_; i++)
    {
      ...
      int theResult = -1;
      theResult = thr_create(0,0,threadFunc_,(void*)this,0,&threads_[i]);
...
#endif
#if defined(HAVE_POSIX_THREADS)
    threads_ = new pthread_t[numThreads_];
    OTCLIB_ASSERT_M(threads_);
    ...
    for (u_int i=0; i<numThreads_; i++)
    {
      ...
      int theResult = -1;
      theResult = pthread_create(&threads_[i],0,threadFunc_,(void*)this);
...
#endif
#if defined(HAVE_WIN32_THREADS)
    threads_ = new HANDLE[numThreads_];
    OTCLIB_ASSERT_M(threads_);
    ...
    for (u_int i=0; i<numThreads_; i++)
    {
      ...
      DWORD threadId;
      threads_[i] = CreateThread(0,0,(LPTHREAD_START_ROUTINE)threadFunc_,
       (LPVOID)this,0,(LPDWORD)&threadId);

      OTCLIB_TRACER(MODULE) << "create thread " << i << " = ";
      OTCLIB_TRACER(MODULE) << threadId << endl;
    }
#endif
  }
...
void OTC_EVThreadPool::shutdown()
{
  ...
  if (shutdown_ == OTCLIB_FALSE)
  {
    shutdown_ = OTCLIB_TRUE;
...
void OTC_EVThreadPool::suspend()
{
  ...
  if (shutdown_ == OTCLIB_FALSE && suspended_ == OTCLIB_FALSE)
...
void OTC_EVThreadPool::queue_(OTC_Event* theEvent, OTC_EVCallback* theCallback)
{
  ...
  if (numThreads_ != 0)
  {
    OTC_Boolean isOverloaded = OTCLIB_FALSE;
...
void* OTC_EVThreadPool::threadFunc_(void* theArg)
{
  ...
  theHandler = (OTC_EVThreadPool*)theArg;
  ...
  while (1)
  {
    ...
    while (
     theHandler->shutdown_ == OTCLIB_FALSE &&
     (theHandler->suspended_ != OTCLIB_FALSE ||
    ...
    if (theHandler->shutdown_ != OTCLIB_FALSE)
    {
      theHandler->exited_++;
