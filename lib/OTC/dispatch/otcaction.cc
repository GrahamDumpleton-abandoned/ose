...
class OSE_EXPORT OTC_ActionCronJob : public OTC_Link
{
  public:
			...
			OTC_ActionCronJob(
			 ...
			 int theAgentData
			  ...
			  : actionId_(theActionId),
			    ...
			    agentData_(theAgentData),
			    minutes_(0,60),
			    minutesWildcard_(OTCLIB_FALSE),
			    hours_(0,24),
			    hoursWildcard_(OTCLIB_FALSE),
			    days_(1,31),
			    daysWildcard_(OTCLIB_FALSE),
			    months_(1,12),
			    monthsWildcard_(OTCLIB_FALSE),
			    daysOfWeek_(0,7),
			    daysOfWeekWildcard_(OTCLIB_FALSE)
				{}

    OTC_Boolean		parse(char const* theSpecification);

    char const*		parseField(
			 char const* theString,
			 OTC_BitSet& theBitSet,
			 OTC_Boolean& theWildcard,
    ...
    OTC_Boolean		isReady(time_t theTime) const;

    OTC_Boolean		isReady(time_t theStart, time_t theEnd) const;
  ...
  private:
    ...
    OTC_Boolean		minutesWildcard_;

    OTC_BitSet          hours_;

    OTC_Boolean         hoursWildcard_;

    OTC_BitSet          days_;

    OTC_Boolean         daysWildcard_;

    OTC_BitSet          months_;

    OTC_Boolean         monthsWildcard_;

    OTC_BitSet          daysOfWeek_;

    OTC_Boolean         daysOfWeekWildcard_;
};
...
OTC_Boolean OTC_ActionCronJob::isReady(time_t theSeconds) const
{
...
OTC_Boolean OTC_ActionCronJob::isReady(time_t theStart, time_t theEnd) const
{
...
OTC_Boolean OTC_ActionCronJob::parse(char const* theSpecification)
{
  char const* theString;
...
OTC_Boolean OTCEV_Action::active(int theActionId)
{
  ...
  if (gSubscriptions_ != 0)
  {
    ...
    {
      OTC_LinkIterator items = gSubscriptions_->items();
      for (items.next(); theStatus==OTCLIB_FALSE
  ...
  if (theEvent->type() == OTCEV_Alarm::typeId())
  {
	...
	for (items1.next(); !items1.isEnd(); items1.next())
	{
	  ...
	  if (theCronJob->isReady(gLastTime_,theTime))
	  {
	    ...
	    theEvent = new OTCEV_Action(OTCLIB_ACTION_CRON,
	     theCronJob->actionId(),theCronJob->description(),
	     theCronJob->agentData());
	    OTCLIB_ASSERT_M(theEvent != 0);

	    theEvent->queue(theCronJob->agentId(),OTCLIB_PRIORITY_JOB);
	  }
	}
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTCEV_Action::matches(char const* theDescription, time_t theTime)
{
