...
void OTC_XmlRpcServlet::processContent(OTC_String const& theContent)
{
  ...
  if (decodeRequest_(theStream,theMethod,
   theObject,theWorkarea) == OTCLIB_FALSE)
  {
...
void OTC_XmlRpcServlet::process(OTC_Event* theEvent)
{
  ...
  if (theEvent->type() == OTCEV_Response::typeId())
  {
	...
	u_int tmpCapacity = tmpMessage->contentLength();

	OTCLIB_TRACER(MODULE) << "content-length = " << tmpCapacity << endl;

	tmpCapacity = u_int(1.5*tmpCapacity);
	...
	if (encodeResponse_(theStream,theObject) != OTCLIB_FALSE)
	{
	  theStream << flush;
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::decodeRequest_(
 istream& theStream,
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::encodeRequest_(
 ostream& theStream,
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::encodeResponse_(
 ostream& theStream,
 OTC_ROPayload const& theObject
)
{
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::encodeObject_(
 ostream& theStream,
 OTC_ROPayload const& theObject
)
{
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::skipProlog_(istream& theStream)
{
  theStream >> ws;
  ...
  {
     if (skipProperty_(theStream) == OTCLIB_FALSE)
       return OTCLIB_FALSE;
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::skipProperty_(istream& theStream)
{
  OTC_Capacity theCapacity(63);
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::decodeValue_(
 istream& theStream,
 OTC_RWPayload theObject,
 OTC_String& theWorkarea
)
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::readName_(
 istream& theStream,
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::readDigits_(
 istream& theStream,
 OTC_String& theResult
)
{
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::readHexDigits_(
 istream& theStream,
 OTC_String& theResult
)
{
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::readToChar_(
 istream& theStream,
 char theChar,
 OTC_String& theResult
)
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::readToString_(
 istream& theStream,
...
)
{
  ...
  while (theInput != EOF)
  {
	...
	theResult += tmpString[u_int(0)];
	tmpString.remove(0,1);

	while (!tmpString.isEmpty())
	{
	  theInput = tmpString[u_int(0)];
...
/* ------------------------------------------------------------------------- */
OTC_Boolean OTC_XmlRpcServlet::decodeToChar_(
 istream& theStream,
 char theChar,
 OTC_String& theResult
)
