...
void OTC_Exchange::messageExchangeAnnounce(OTCEV_Envelope* theEnvelope)
{
  ...
  theRemoteAddress = theEnvelope->previousSender();
  ...
  if (theEndPoint != 0)
  {
    ...
    else
    {
      ...
      OTC_String theRemoteProcess;
      theRemoteProcess = theEnvelope->message()->content();
...
void OTC_Exchange::handleBroadcastMessage(OTCEV_Envelope* theEnvelope)
{
  ...
  OTC_Boolean tmpServerBounce;
  tmpServerBounce = OTCLIB_FALSE;

  if (theRegistry != 0)
  {
    if (theRegistry->exchangeType() == OTCLIB_EXCHANGE_CLIENT)
      tmpServerBounce = OTCLIB_TRUE;
  }
